  ...
  const SyntaxHighlighting = {
    init() {
      ...
      codeBlocks.forEach(pre => {
        ...
        const hasHighlighting = pre.querySelector('span.org-keyword, span.org-string, span.org-comment');
        if (!hasHighlighting) {
    ...
    applySyntaxHighlighting(pre, lang) {
      ...
      const keywords = {
        go: ['package', 'import', 'func', 'type', 'struct', 'interface', 'var', 'const', 'if', 'else', 'for', 'range', 'switch', 'case', 'default', 'return', 'break', 'continue', 'defer', 'go', 'chan', 'make', 'new', 'nil', 'true', 'false'],
        rust: ['use', 'pub', 'fn', 'let', 'mut', 'struct', 'impl', 'trait', 'enum', 'match', 'if', 'else', 'for', 'while', 'loop', 'return', 'break', 'continue', 'self', 'Self', 'super', 'crate', 'mod', 'true', 'false', 'Some', 'None', 'Ok', 'Err', 'async', 'await', 'move', 'ref', 'where', 'type', 'const', 'static', 'unsafe', 'extern']
      };
      
      const types = {
        go: ['string', 'int', 'int8', 'int16', 'int32', 'int64', 'uint', 'uint8', 'uint16', 'uint32', 'uint64', 'float32', 'float64', 'bool', 'error', 'byte', 'rune', 'interface{}', 'any'],
        rust: ['String', 'str', 'i8', 'i16', 'i32', 'i64', 'i128', 'u8', 'u16', 'u32', 'u64', 'u128', 'f32', 'f64', 'bool', 'char', 'Vec', 'HashMap', 'Option', 'Result', 'Box', 'Rc', 'Arc', 'Cell', 'RefCell', 'Mutex', 'RwLock']
      };
    ...
    highlightKeywordsAndTypes(text, lang) {
      ...
      if (lang === 'json' || lang === 'yaml') {
        ...
        if (lang === 'yaml') {
          text = text.replace(/^(\s*)(\w+)(?=\s*:)/gm, '$1<span class="org-variable-name">$2</span>');
